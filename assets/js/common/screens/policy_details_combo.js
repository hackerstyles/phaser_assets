...
const polDet_Combo_SCREENSHOT_NAME = 'policy_details_ss';
var polDet_Combo_NEXT_SCREEN;
...
async function screenshot_polDet()
{
  ...
  var result = await call_IMAGE_SAVE_URL_API(getScreenImgDataURL(),polDet_Combo_SCREENSHOT_NAME,window.proposal_no,'screenshot',window.latitude_value,window.longitude_value,window.sysLang);

}

async function next_polDet(choice1,screen_name)
{

image_count++;
if((sysLang=='eng') && (cur_scr==13))
{
  if(image_count==1){
...
agree_count++;
    ...
    if(Object.keys(window.params.rider_details).length >0)
    {
      if(agree_count==1){
        var result = await screenshot_polDet();
    var result1=await  userAgree_polDet(window.proposal_no,'agreed','policy_details');
           //polDet_NEXT_SCREEN = 6;
           goToPage(6);
             image_count=0;
           agree_count=0;
  ...
  agree_count++;
    ...
    if(Object.keys(window.params.rider_details).length >0)
    {
      if(agree_count==1){
         var result = await screenshot_polDet();
    var result1=await userAgree_polDet(window.proposal_no,'disagreed','policy_details');
           //polDet_NEXT_SCREEN = 6;
           goToPage(14);
             image_count=0;
           agree_count=0;
...
async function userAgree_polDet(proposal, choice, screen)
{
  ...
  var result = await call_AGREE_STATUS_URL_API(proposal, choice, 'policy_details');
